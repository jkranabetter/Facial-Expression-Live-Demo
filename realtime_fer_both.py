...
'''
Emotion Recognition
This program pitts the neural net vs the svm
...
svm_emotions = ['Anger', 'Disgust', 'Fear',
                'Happy', 'Sadness', 'Surprise', 'Neutral']
neural_emotions = ["Anger", "Disgust", "Fear",
...
while (webcam.isOpened()):
    ...
    for (x, y, w, h) in faces:
        ...
        # svm stuff
        hog_features, hog_image = hog(face_crop, orientations=8, pixels_per_cell=(
        ...
        if svm_max_value > 0.5:
            ...
            print("SVM: " + svm_emotion + " confidence level of " +
                  str(svm_max_value*100) + "%")

        # update neural labels
        if neural_max_value > 0.5:
            neural_emotion = neural_emotions[np.where(
                neural_results == neural_max_value)[0][0]]
            print("Neural: " + neural_emotion +
                  " confidence level of " + str(neural_max_value*100) + "%")

        # text on image
        cv2.putText(image, svm_emotion, (x + 6, y - 6),
                    font, 1.0, (255, 255, 255), 1)
        cv2.putText(image, neural_emotion, (x + 6, y - 6 +
                    image_height), font, 1.0, (255, 255, 255), 1)
        cv2.putText(image, 'Support Vector', (0, 25),
                    font, 1.0, (255, 255, 255), 1)
        cv2.putText(image, 'Neural Network', (0, image_height + 25),
                    font, 1.0, (255, 255, 255), 1)
